...
case "$target" in
  pill|i3) ;;
  *) echo "toggle-profile: unknown profile '$target' (pill|i3)" >&2; exit 1 ;;
esac

aero="$HOME/.config/aerospace/aerospace.toml"
frag="$HOME/.config/aerospace/gaps.${target}.toml"

[[ -f "$frag" ]] || { echo "toggle-profile: missing $frag" >&2; exit 1; }
[[ -f "$aero" ]] || { echo "toggle-profile: missing $aero" >&2; exit 1; }
