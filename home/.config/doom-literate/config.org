...
For some strange reason, the following variable has recently been causing issues
when starting LSP servers. Let's just set to =nil= for now and see if we can
...
Proteolas also developed a great themes pack by the name of *EF Themes*.
...
For syncing file changes from tangled files back to this config.
#+begin_src emacs-lisp :tangle packages.el
...
ely though, =accsupp= doesn't seem to accept five digit hexadecimal
