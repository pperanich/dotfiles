_: {
      ...
      config = {
        ...
        services.unbound = {
          ...
          settings = {
            ...
            # Forward zones
            forward-zone = lib.mapAttrsToList (zone: servers: {
              name = zone;
        ...
        # Firewall
        networking.firewall.allowedTCPPorts = lib.mkIf (builtins.any (
          addr: addr != "127.0.0.1" && addr != "::1"
        ) cfg.listenAddresses) [ 53 ];
        networking.firewall.allowedUDPPorts = lib.mkIf (builtins.any (
          addr: addr != "127.0.0.1" && addr != "::1"
        ) cfg.listenAddresses) [ 53 ];
  ...
  flake.modules.homeManager.unbound =
    { pkgs, ... }:
    {
